...
unsigned char oled_string[80];//vetor para os caracteres a serem escritos no display, 21 caracteres mais o '\0' no final.
...
void Oled_Printf (int linha, const char *format,...){
	static char oled_init=0;
	char len;
	va_list args;
  va_start(args, format);
	len=vsprintf((char*)oled_string,format,args);
	va_end(args);
	for(;len<=LineLen;len++)oled_string[len]=' ';
	oled_string[LineLen]=0;
	if(oled_init==0){Oled_Init();oled_init=1;}
	Print_String(linha-1,0,oled_string);
	Oled_Refresh();
}
